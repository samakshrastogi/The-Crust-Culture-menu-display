...
export default function SearchBar({ value, onChange }) {
  return (
    <label className="flex h-10 w-full min-w-0 items-center gap-2 rounded-xl border border-[var(--line)] bg-[var(--surface)] px-3 text-[var(--muted)] shadow-xs transition-colors focus-within:border-[var(--orange)] sm:h-11 sm:gap-2.5 sm:px-3.5">
      <FiSearch className="shrink-0 text-sm sm:text-base text-[var(--gold)]" aria-hidden="true" />
      <input
        value={value}
      ...
      {value && (
        <button
          type="button"
          onClick={() => onChange('')}
          className="touch-target grid place-items-center rounded-full text-[var(--muted)] cursor-pointer hover:text-[var(--text)]"
